	...
	class AddThemePage {
		...
		public function add_menu_in_admin_menu() {
			...
			if ( "main" == $this->get_menu_type() ) {
				add_theme_page( $this->get_menu_name(), $this->get_menu_title(), $this->get_capability(), $this->get_menu_slug(), array(
					$this,
					'display_function'
				), $this->get_menu_icon() );
			}
			...
			if ( "sub" == $this->get_menu_type() ) {
				add_submenu_page( $this->data['cpt_menu'], $this->get_menu_title(), $this->get_menu_title(), $this->get_capability(), $this->get_menu_slug(), array(
					$this,
					'display_function'
				) );
			}
			...
			foreach ( $this->get_pages() as $panelsIndex => $panels ):
				...
				foreach ( $panels['page_settings'] as $sectionIndex => $sections ):
					...
					foreach ( $sections['options'] as $option ) :
						...
						add_settings_field( $option['id'], $option['title'], array(
							$this,
							'field_generator'
						), $panelsIndex, $sectionIndex, $option );
										...
										foreach ( $page_settings as $section_id => $nav_sections ):
                                            ...
                                            <li>
                                                ...
                                                <a sectionId="<?php echo $section_id; ?>"
                                                   dataid="<?php echo $page_id; ?>"
                                                   href='#<?php //echo $pagenow.'?'.$nav_menu_url;
												   ?><?php echo
												   $page_id; ?>'
                ...
                <div class="form-wrapper">
                    ...
                    <form class="" action='options.php' method='post'>
                        ...
                        <div class="form-section">
							...
							foreach ( $this->get_pages() as $panelsIndex => $panels ):
								?>
                                <div class="tab-content <?php if ( $current_page == $panelsIndex ) {
									echo 'active';
								} ?>  tab-content-<?php echo
								$panelsIndex;
								?>">
									<?php
