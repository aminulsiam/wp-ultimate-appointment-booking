...
class Mage_Plugin_Public {
	...
	private function load_public_dependencies() {
		require_once MAGE_PLUGIN_DIR . 'public/shortcode/shortcode-hello.php';
	}

	public function enqueue_styles() {
		wp_enqueue_style( 'mage-public-css', MAGE_PLUGIN_DIR . 'css/style.css', array(), time(), 'all' );

	}


	public function enqueue_scripts() {
		wp_enqueue_script( 'mage-public-js', MAGE_PLUGIN_DIR . 'js/mage-plugin-public.js', array( 'jquery' ), time(), false );
	...
	public function mage_register_custom_single_template($template) {
		global $post;
		  if ($post->post_type == "mage_video"){
		          ...
		          $default_path = MAGE_PLUGIN_DIR. 'public/templates/'; 
	...
	public function mage_register_custom_tax_template( $template ){
	    if( is_tax('mage_video_cat')){
	        $template = MAGE_PLUGIN_DIR.'public/templates/taxonomy-category.php';
	    }    
	    return $template;
	}
